...
async def test_budget_full_flow(client: AsyncClient):
    ...
    # 2. Create Workspace
    ws_resp = await client.post("/workspaces", json={"name": "E2E WS"}, headers=headers)
    workspace_id = ws_resp.json()["id"]

    # 3. Get Categories
    cat_resp = await client.get("/budgets/categories", headers=headers)
    ...
    response = await client.post("/budgets", json=budget_data, headers=headers)
    ...
    # 6. List Budgets
    response = await client.get(f"/budgets?workspace_id={workspace_id}", headers=headers)
    assert response.status_code == 200
    data = response.json()
    assert data["total"] >= 1
    ...
    # 7. Update Budget
    response = await client.put(f"/budgets/{budget_id}", json={"limit_amount": 1200.0}, headers=headers)
    assert response.status_code == 200
